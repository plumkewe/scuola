...
int main() {
	...
	int aNumericasuali[21], //  creiamo un array di tipo int e con "10 elementi"
	sDove, sColpita = 0,
	sTentativo = 1,
	sQuanti = sizeof(aNumericasuali)/sizeof(aNumericasuali[0]); //  troviamo quanti elementi ci sono nel array
	
	
	fill (aNumericasuali, aNumericasuali + sQuanti, 0); //  riempie l'array con il valore '0'
	...
	for (short int i = 0; i < 4; ++ i) { //  per posizionare le tre navi in tre posizioni casuali da 0 a 20
		...
		aNumericasuali[rand()%20] = 1; //  assegno al elemento casuale il valore 1 (la nave ce)
	...
	while (sTentativo <= 20) { //  svolgo questo blocco di codice affinche i tentativi non supperano il (pari) 20
		
		cout << "[!] è il tuo " << sTentativo << " tentativo, secondo te dove si trova la nave? Y1 X";
		cin >> sDove;
		
		if (sDove < 0 || sDove > 20) { //  nel caso in cui se uno esce dal range dovrà pagare il doppio
		...
		if (aNumericasuali[sDove] == 1) { //  verifico se nel numero che l'utente abbia inserito è presente una nave
